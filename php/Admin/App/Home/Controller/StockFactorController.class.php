 ...
 * “帖子标签”控制器类。
 */ 
class StockFactorController extends CommonController {
	...
	public function save(){
        ...
        $data['operator'] = $res_isLogin;

		if (!$data['id'] || !$data['ex_date'] || !$data['detail'] || !$data['current_factor']) {
        ...
        if ($list && $list != null) {
        	if ($list[0]['ex_date'] != $data['ex_date']	) {
                ...
                if (!$LEX) {
                    ...
                    if($LSave){
                        $LData['msg'] = "复权数据: code ".$data['code']." 复权数据修改成功";
                        aLog($LData);
                ...
                for ($i=0; $i < $num; $i++) {
                    ...
                    $LSave = $Model->where("1=1 and id='".$LEX[$i]['id']."'")->save($dataFinal);
                    if(!$LSave){
                        $LData['msg'] = "复权数据: id ".$LEX[$i]['id']." 复权数据(生效时间更改)修改失败";
                        aLog($LData);
                ...
                if(!$LSave){
                    ...
                    if (!$LSaveTemp) {
                        $LData['msg'] = "复权数据: id ".$data['id']." 复权数据(复权因子)修改失败";
                        aLog($LData);
                ...
                if(!$LSave){
                    $LData['msg'] = "复权数据: id ".$data['id']." 复权数据(分类或者描述)修改失败";
                    aLog($LData);
                    $this->ajaxOutput(-1, '修改失败', array(count=>count($LSave),'list'=>$LSave));
                }else{
                    $LData['msg'] = "复权数据: id ".$data['id']." 复权数据(分类或者描述)修改成功";
                    aLog($LData);
                    $this->ajaxOutput(0, '修改成功', array(count=>count($LSave),'list'=>$LSave));
                }        
            }
	...
	public function add(){
		...
		if (!$data['code'] || !$data['ex_date'] || !$data['detail'] || !$data['current_factor']) {
			$this->ajaxOutput(-1, '提交的数据不完整', array(count=>0,'list'=>array()));
		}
        ...
        if ($list || $list == null) {
        	...
        	if ($data['ex_date']>=$list[$num-1]['ex_date']) {
                ...
                $LData['msg'] = "复权数据: code ".$data['code']." 复权数据添加成功";
                aLog($LData);
    		 	$this->ajaxOutput(0, '添加成功！', array(count=>count($LAdd),'list'=>$LAdd));
    		}
